...
async def faq_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    faq_text = (
        "Мы - конференция AURA и мы будем рады новым людям.\n\n"
        "FAQ по чату:\n\n"
        "- Есть ли здесь место для меня? \n"
        "Мы рады новым участникам, которые разделяют наши интересы и готовы общаться на равных. Если вы пришли просто за вниманием или ищете новых знакомств ради развлечений, то это не наш чат.\n"
        "- Есть ли система предупреждений? \n"
        "Нет, мы сразу принимаем решения о бане.\n"
        "- Какие у вас есть боты? \n"
        "У нас есть бот Chat Norris, который делает саммари последних 400 сообщений, имеет встроенный GPT и функцию «прожарки» постов. Также есть Iris, который приветствует новеньких и помогает банить. И есть Курилкабот, который я создаю и тестирую сам. Ну и этот бот, в котором вы сейчас читаете этот текст.\n"
        "- А расшифровщик голосовых есть? \n"
        "В группе достаточно голосов, расшифровка голосовых сообщений доступна всем участникам, в том числе и без подписки Премиум в ТГ.\n"
        "- Откуда вы взялись и чем живете? \n"
        "Мы появились в ВК около 8 лет назад. Изначально это была ролевая конфа с множеством участников, но со временем мы убрали фейков и фриков, вектор развития переместился на реал и чат стал более дружелюбным. В 2021 году мы переехали в ТГ. За все время у нас сменилось около тысячи человек. Мы стали встречаться в реале, дарить подарки на дни рождения и поняли, что мы не просто чувачки в интернете, а настоящие друзья. И стараемся пополнять этот список друзей."
    )
    keyboard = [["Заполнить анкету", "Хочу прочесть FAQ"]]
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=False, resize_keyboard=True)
